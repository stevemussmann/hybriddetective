...
hybridPowerComp3 <-function(dir,filetag="",Thresholds=c(0.5,0.6,0.7,0.8,0.9),addThresh=FALSE,samplesize=NULL,CT=0.1,CTI=0.5){
  ...
  temp2 <- temp
  tempAccuracy <- temp
  tempAccuracy$isgood=TRUE
  tempAccuracy$domatch=FALSE
  ...
  accuracy_boxplot <- ggplot(filter(AccuracyData,pofz %in% c(0.5,0.75,0.9)), aes(x = max.class, y = means, fill = max.class)) +
  ...
  #Summary among simulations --
      ...
      ## line plot - accuracy with SD
       accuracy_lineplotSD <- ggplot(SummaryAccuracy) +
           geom_line(aes(x = pofz, y = mean, colour = max.class), lwd = 1.25) +
           geom_line(aes(y = mean+sd, x = pofz, colour = max.class), linetype = 2) +
           geom_line(aes(y = mean-sd, x = pofz, colour = max.class), linetype = 2) +
           facet_grid(.~nLoci) + theme(panel.background = element_rect(fill = "white", colour = "black"),
                                      plot.background = element_rect(colour = "white"),
                                      panel.grid.major = element_line(colour = "grey90"),
          legend.position="bottom", strip.background = element_rect(colour = "black", fill = "white")) +
          scale_color_brewer(palette = "Dark2")+
          labs(x = "Critical PofZ Threshold", y = expression("Proportion of Assignments Correct "%+-%"sd"),
               col="Genotype Frequency Class") + ylim(0, 1)
    ...
    #   {write.csv(final.stats.pofzeds, paste0(dir,"Figures and Data/data/AccuracyBoxPlotData.csv"), row.names = FALSE, quote = FALSE)}
       ...
       accuracy_lineplot <-  ggplot(SummaryAccuracy) +
           geom_line(aes(x = pofz, y = mean, colour = max.class), lwd = 1.25) +
         scale_color_brewer(palette = "Dark2")+
          facet_grid(.~nLoci) +
         theme(panel.background = element_rect(fill = "white", colour = "black"),
      ...
      accuracy_lineplot_ClassFacet <-  ggplot(SummaryAccuracy) +
        geom_line(aes(x = pofz, y = mean, colour = nLoci), lwd = 1.25) +
        facet_grid(.~max.class) +
        theme(panel.background = element_rect(fill = "white", colour = "black"),
              plot.background = element_rect(colour = "white"), panel.grid.major = element_line(colour = "grey90"),
     ...
     Accuracy_ByThreshold_LinePlot_AllClass <-ggplot(dplyr::filter(SummaryAccuracy,pofz %in% Thresholds),
                                                   aes(x=factor(nLoci),y=mean,col=max.class,group=max.class))+
                                              ...
                                              labs(x="Panel Size (Loci)",y=expression("Proportion of Assignments Correct "%+-%"sd"),
                                                   col="Genotype Frequency Class",group="")+scale_color_brewer(palette = "Dark2")+
                                              theme(panel.background = element_rect(fill = "white", colour = "black"),
                                                    plot.background = element_rect(colour = "white"),
                                                    panel.grid.major = element_line(colour = "grey90"),
  ...
  writeLines("
    ...
    ### TYPE I ERROR
       ...
       colnames(TypeIout)=c("PofZ","Loci","Prop")
        ...
        TypeI_BoxPlot <- ggplot(filter(TypeIout.PofZeds,PofZ %in% c("0.5","0.75","0.9")), aes(x = PofZ, y = Prop)) + geom_boxplot(fill = "grey75") + facet_grid(.~Loci) +
    ...
    if(filetag!=""){write.csv(TypeIout.PofZeds, paste0(dir,"Figures and Data/data/", filetag,"_TypeIBoxPlotData.csv"), row.names = FALSE, quote = FALSE)}else
      {write.csv(TypeIout.PofZeds, paste0(dir,"Figures and Data/data/TypeIBoxPlotData.csv"), row.names = FALSE, quote = FALSE)}
        ...
        max.y = max(testsum_TypeI$means + testsum_TypeI$sd)
         ...
         typeI_lineplot <-
           ggplot(testsum_TypeI) + geom_line(aes(x = as.numeric(PofZ), y = means, group = factor(Loci)), size = 2) +
           geom_line( aes(y = (means+sd), x = as.numeric(PofZ), colour = Loci), lty = 2) +
           geom_line( aes(y = (means-sd), x = as.numeric(PofZ), colour = Loci), lty = 2) +
           facet_grid(.~Loci) +  ylim(ymin = 0, ymax = max.y) +
  ...
  legend.position = "none", strip.background = element_rect(colour = "black", fill = "white"), text = element_text(colour = "black"))
    ...
    miss_mean <- missout%>%group_by(nLoci,pofz,class)%>%
    ...
    PlotData <- melt(FinalData3[c("nLoci","level","class","mprobP1","mprobP2","mprobF1","mprobF2","mprobBC1","mprobBC2")],
                     id.vars=c("nLoci","level","class"))
    PlotDatasd <- melt(FinalData3[c("nLoci","level","class","sdprobP1","sdprobP2","sdprobF1","sdprobF2","sdprobBC1","sdprobBC2")],
                       id.vars=c("nLoci","level","class"))
