...
#' @importFrom reshape2 melt
 ...
 nh_plotR <- function(NHResults, ColourVector = c("red", "blue", "grey", "green", "black", "yellow", "brown"), ReversePure = 1){ ## this function plots the Q-value output from New Hybrids
  ...
  pretty.plot <- ggplot(NH_melt, aes(x = Indv, y=CumProb, fill=PopProb))
    ...
    pretty.plot+geom_bar(stat="identity", position = "stack", width = 1) + scale_fill_manual(values=col.vec)+scale_colour_manual(values=ColourVector)+ylab("Cumulative Probability")+xlab("Individual") +
