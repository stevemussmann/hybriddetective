...
#' @param ColourVector A logical which can be used to reverse the Pure1/BC1, Pure2/BC2 colours if NEWHYBRIDS switches which population is called Population1 and Population2 between analyses of the same data (allows you to make your plots the same colours). Options are "1" or "2"
#' @export
#' @import ggplot2
#' @importFrom reshape2 melt
 ...
 nh_plotR <- function(NHResults, ColourVector = 1){ ## this function plots the Q-value output from New Hybrids
  ...
  pretty.plot <- ggplot(NH_melt, aes(x = Indv, y=CumProb, fill=PopProb))
    ...
    pretty.plot+geom_bar(stat="identity", position = "stack", width = 1) + scale_fill_manual(values=col.vec)+scale_colour_manual(values=col.vec)+ylab("Cumulative Probability")+xlab("Individual") +
