  ...
  for (z in classnames){
    ...
    if(nrow(x2)>0){
      ...
      xout <- data.frame(class=z,
                         mclass_P1=x4[which(temp4$Var1 == "Pure1"),"Freq"],
                         mclass_P2=x4[which(temp4$Var1 == "Pure2"),"Freq"],
                         mclass_F1=x4[which(temp4$Var1 == "F1"),"Freq"],
                         mclass_F2=x4[which(temp4$Var1 == "F2"),"Freq"],
                         mclass_BC1=x4[which(temp4$Var1 == "BC1"),"Freq"],
                         mclass_BC2=x4[which(temp4$Var1 == "BC2"),"Freq"])
    } else
    {xout <- data.frame(class=z,
                        mclass_P1=NA,
                        mclass_P2=NA,
